/* PL_With_C_ViaArray.java (Positional List with Cursors via array)
**
** Java class that implements the PL_With_C (Positional List with Cursors)
** interface, in which the list has an array-based representation.
**
** @author R. McCloskey
** @version April 2012
**/

public class PL_With_C_ViaArray<T> implements PL_With_C<T> 
{
...
*/
   ...
   public String toString()
   {
      ...
      if (isEmpty()) { 
         ...
         result = new String(")");
         PLC<T> c = getCursor().toRear().toPrev();
   ...
   */
   public PLC<T> getCursor() { return new Cursor(this); }
   ...
   private class Cursor implements PLC<T> {

      /**** instance variables ****/

      private PL_With_C_ViaArray<T> myList;  // this cursor's list
      private int loc;   // array index corresponding to cursor's position
      

      public Cursor(PL_With_C_ViaArray<T> list)
      {
         myList = list;
         loc = 0;  /* rear */
      ...
      */
      ...
      public T getItem() throws PLC_Exception
      {
         if (atRear()) {
            throw new PLC_Exception("getItem() requires !atRear()"); 
         }
         return myList.contents[loc];
      }

 
      public boolean equals(PLC<T> cur)  
      {
         Cursor c;
      ...
      public PL_With_C<T> getList() { return myList; }


      /* navigation mutators */

      public PLC<T> toFront()
         { return changeLocTo(myList.frontLoc); }

      public PLC<T> toRear() 
         { return changeLocTo(0); }

      public PLC<T> toNext() throws PLC_Exception 
      {
         if (atRear()) { 
            throw new PLC_Exception("toRear() requires !atRear()"); 
         }
         return changeLocTo(myList.next[loc]);
      }

      public PLC<T> toPrev() throws PLC_Exception
      {
         if (atFront()) {
            throw new PLC_Exception("toFront() requires !atFront()"); 
         }
         return changeLocTo(myList.prev[loc]);
      }


      public PLC<T> setTo(PLC<T> cur)
      {
         Cursor c;
      ...
      public T remove() throws PLC_Exception
      {
         T result;
         
         if (atRear()) {
            throw new PLC_Exception("remove() requires !atRear()"); 
         }

         if (myList.refCntr[loc] > 1) {
            throw new PLC_Exception("cannot remove node at which there are multiple cursors");
         }
      ...
      public T replace(T item) throws PLC_Exception
      {
         T result; 
         if (atRear()) {
            throw new PLC_Exception("replace() requires !atRear()");
         }
      ...
      */
      private PLC<T> changeLocTo(int newLoc) {
         myList.refCntr[loc]--;
         loc = newLoc;
         myList.refCntr[loc]++;
         return this;
